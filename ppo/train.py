    ...
    for _ in range(eval_episodes):
        ...
        while not done:
            with torch.no_grad():
                ...
                # Availability mask
                availability_mask = torch.tensor(state[:num_actions], dtype=torch.float32).unsqueeze(0)
                probs = probs * availability_mask

                # Early bias
                decay = torch.exp(-torch.arange(num_actions, dtype=torch.float32) / 30.0).unsqueeze(0)
                probs = probs * decay
